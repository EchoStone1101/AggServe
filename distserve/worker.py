...
@ray.remote(num_cpus=0, num_gpus=1.0)
class ParaWorker:
    ...
    def init_kvcache_and_swap(self, num_gpu_blocks, num_cpu_blocks) -> (cudaMemoryIpcHandle, cudaMemoryIpcHandle):
        ...
        logger.info(f"Worker {self.worker_id}@{self.stage} switching CUDA context {cuda.cuCtxGetCurrent()} device {cuda.cuCtxGetDevice()}")
...
class MPSParaWorker:
    ...
    def __init__(
        ...
        # logger.info(f"MPSWorker {self.worker_id}@{self.stage} environ: {os.environ}")
        ...
        if self.mps_precentage is not None:
            ...
            mps_precentage = int(round(100 * self.mps_precentage))
            logger.info(f"MPSWorker {worker_id}@{stage} setting MPS percentage to {mps_precentage}")
            ...
            logger.info(f"MPSWorker {worker_id}@{stage} switching CUDA context from {prev_ctx} to {cur_ctx}")
    ...
    def ready(self):
        ...
        If ready is called, the actor is ready.
        """
        logger.info(f"MPSWorker {self.stage}.#{self.worker_id} created on host {socket.gethostname()} and gpu #{self.gpu_id}")
        if value := os.getenv("CUDA_MPS_ENABLE_PER_CTX_DEVICE_MULTIPROCESSOR_PARTITIONING", None):
            logger.info(f"MPSWorker {self.stage}.#{self.worker_id} has ENV={value}")
        else:
            logger.info(f"MPSWorker {self.stage}.#{self.worker_id} does not see ENV")
        pass
    ...
    def init_kvcache_and_swap(self, num_gpu_blocks, num_cpu_blocks) -> (cudaMemoryIpcHandle, cudaMemoryIpcHandle):
        ...
        logger.info(f"MPSWorker {self.worker_id}@{self.stage} switching CUDA context {cuda.cuCtxGetCurrent()} device {cuda.cuCtxGetDevice()}")
    ...
    def _profile_num_available_blocks(
        ...
        logger.info(f"runtime peak memory: {peak_runtime_memory / GB:.3f} GB")
        logger.info(f"total GPU memory: {total_gpu_memory / GB:.3f} GB, available GPU memory: {(total_gpu_memory * gpu_memory_utilization - peak_runtime_memory) / GB:.3f}")
    ...
    def step(
        ...
        start = time.time()
        # print(f"MPSWorker {self.stage}.#{self.worker_id} Step begin")
        # run forward
        generated_tokens_ids = self.model.forward(
            input_tokens_batched,
            first_token_indexes,
        ...
        self.execution_time += time.time() - start
        # print(f"MPSWorker {self.stage}.#{self.worker_id} Step end")
